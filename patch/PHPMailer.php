...
class PHPMailer
{
    ...
    /**
     * Send messages using sendViaGraph() function.
     */
    ...
    {
	$plugin = \Plugin::model()->findByAttributes(['name' => 'GraphMailer']);

	// Check if the Graph client is configured
	if ($plugin) {
            $pluginId = $plugin->id;
	    $clientId = $this->getPluginSetting($pluginId, 'clientId');
	}
    ...
    public function getPluginSetting(string $plugin, string $key, $default = null)
    {
    ...
    /**
     * Send mail using the Graph API.
     *
     * @throws Exception
     *
     * @return bool
     */
    protected function sendViaGraph($mailer): bool
    {
	// Get the plugin id 
        $plugin = \Plugin::model()->findByAttributes(['name' => 'GraphMailer']);
	...
	$tenantId = '';
	    ...
	    $clientSecret = $this->getPluginSetting($pluginId, 'clientSecret');
	    $clientSecret = str_replace('"', '', $clientSecret);
            $tenantId = $this->getPluginSetting($pluginId, 'tenantId');
	    $tenantId = str_replace('"', '', $tenantId);
        }

	$to = array_map(fn($a) => [
            'emailAddress' => ['address' => $a[0]]
	...
	$to_email = $to[0]['emailAddress']['address'] ?? null;

        if ($to_email !== null) {
            echo $to_email;
        }
	$cc_email = $cc[0]['emailAddress']['address'] ?? null;

        if ($cc_email !== null) {
            echo $cc_email;
	}
	$bcc_email = $bcc[0]['emailAddress']['address'] ?? null;

        if ($bcc_email !== null) {
            echo $bcc_email;
        }

        // Access PHPMailer internals safely
        $fromUser    = $this->From;
	...
	//
        ...
        if (!$token) {
            echo "Token response did not contain an access token.\n";
            exit(1);
        }
        ...
        $emailJson = json_encode($emailData, JSON_UNESCAPED_SLASHES);

        // ==== 3. Send the Email ====
        $mailUrl = "https://graph.microsoft.com/v1.0/users/$fromUser/sendMail";
        ...
        if ($httpCode !== 202) {
            ...
            // Optional: parse Graph error details
            $error = json_decode($response, true);
            if (isset($error['error']['message'])) {
                echo "Graph API error: " . $error['error']['message'] . "\n";
            }
            exit(1);
        }
